...
int do_everything(int argc, LPCWSTR argv[]) {
	...
	threadArgs.leftSignalChain = std::make_shared<DSP::SignalChain>();
	threadArgs.rightSignalChain = std::make_shared<DSP::SignalChain>();

	threadArgs.leftSignalChain->addFilter(std::make_unique<DSP::Filter::Biquad::PeakingEQ>
		(160.f, 1.5f, -3.f));
	threadArgs.rightSignalChain->addFilter(std::make_unique<DSP::Filter::Biquad::PeakingEQ>
		(160.f, 1.5f, -3.f));
	threadArgs.leftSignalChain->addFilter(std::make_unique<DSP::Filter::Biquad::PeakingEQ>
		(1250.f, 2.f, -2.f));
	threadArgs.rightSignalChain->addFilter(std::make_unique<DSP::Filter::Biquad::PeakingEQ>
	...
	threadArgs.rightSignalChain->addFilter(std::make_unique<DSP::Filter::Biquad::PeakingEQ>
		(17000.f, 1.f, -6.3f));
	threadArgs.leftSignalChain->addFilter(std::make_unique<DSP::Filter::Biquad::HighShelf>
		(100.f, 0.7071f, -2.5f));
	threadArgs.rightSignalChain->addFilter(std::make_unique<DSP::Filter::Biquad::HighShelf>
		(100.f, 0.7071f, -2.5f));
