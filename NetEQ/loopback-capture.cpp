...
HRESULT LoopbackCapture(
    ...
    HANDLE hStopEvent,
    PUINT32 pnFrames,
	std::shared_ptr<DSP::SignalChain> leftSignalChain,
	std::shared_ptr<DSP::SignalChain> rightSignalChain
);
...
DWORD WINAPI LoopbackCaptureThreadFunction(LPVOID pContext) {
    ...
    pArgs->hr = LoopbackCapture(
        ...
        pArgs->hStopEvent,
        &pArgs->nFrames,
		pArgs->leftSignalChain,
		pArgs->rightSignalChain
    );
...
HRESULT LoopbackCapture(
	...
	//Prepare signal chain
	leftSignalChain->setSampleRate(pwfx->nSamplesPerSec);
	rightSignalChain->setSampleRate(pwfx->nSamplesPerSec);
	...
	get_specific_device(L"Speakers (High Definition Audio Device)", &outputDevice);
	WASAPIOutput audioOutput{ outputDevice, *pwfx };

	std::cout << "[Info] Bits per sample: " << pwfx->wBitsPerSample << ", Channels: " << pwfx->nChannels << ", Sample Rate: "
		<< pwfx->nSamplesPerSec << ", Block Align: " << pwfx->nBlockAlign << std::endl;

	if (pwfx->wBitsPerSample != 16 || pwfx->nChannels != 2) {
    ...
    *pnFrames = 0;
    ...
    for (UINT32 nPasses = 0; !bDone; nPasses++) {
        ...
        for (
			...
			DSP::SampleBuffer leftSamples(nNumFramesToRead), rightSamples(nNumFramesToRead);
			for (size_t i = 0; i < nNumFramesToRead; ++i) {
				int offset = nBlockAlign * i;

				memcpy(&rightSamples[i], pData + offset, 2);
				memcpy(&leftSamples[i], pData + offset + 2, 2);
			}

			leftSignalChain->processSamples(leftSamples);
			rightSignalChain->processSamples(rightSamples);

			audioOutput.writeSamples(leftSamples, rightSamples);

			for (size_t i = 0; i < nNumFramesToRead; ++i) {
				int offset = nBlockAlign * i;

				memcpy(pData + offset, &rightSamples[i], 2);
				memcpy(pData + offset + 2, &leftSamples[i], 2);
			}
